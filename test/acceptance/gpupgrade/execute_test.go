...
func TestExecute(t *testing.T) {
	...
	t.Run("gpupgrade execute should remember that link mode was specified in initialize", func(t *testing.T) {
		...
		executeQuery(t, source.Connection(), fmt.Sprintf(`CREATE TABLE %s (a int);`, table))
		defer executeQuery(t, source.Connection(), fmt.Sprintf(`DROP TABLE IF EXISTS %s;`, table))

		sourceRelfilenodes := getRelfilenodes(t, source.Connection(), source.Version, table)
		for _, relfilenode := range sourceRelfilenodes {
			numHardlinks := getNumHardLinks(t, relfilenode)
			if numHardlinks != 1 {
				t.Fatalf("got %q want %q hardlinks", numHardlinks, 1)
			}
		}
		...
		for _, relfilenode := range intermediateRelfilenodes {
			numHardlinks := getNumHardLinks(t, relfilenode)
			if numHardlinks != 2 {
				t.Fatalf("got %q want %q hardlinks", numHardlinks, 2)
			}
		}
	})
}
...
CREATE FUNCTION pg_temp.gp_relation_filepath(tbl text)
	...
	if err != nil {
		t.Fatalf("executing sql failed: %v", err)
	}
	defer rows.Close()
