...
func (s *Server) InitializeCreateCluster(req *idl.InitializeCreateClusterRequest, stream idl.CliToHub_InitializeCreateClusterServer) (err error) {
	...
	st.AlwaysRun(idl.Substep_check_upgrade, func(stream step.OutStreams) error {
		...
		if err := UpgradeCoordinator(stream, s.BackupDirs.CoordinatorBackupDir, req.GetPgUpgradeVerbose(), req.GetSkipPgUpgradeChecks(), req.GetJobs(), s.Source, s.Intermediate, idl.PgOptions_check, s.Mode, pgUpgradeTimestamp); err != nil {
			return err
		}

		return UpgradePrimaries(s.agentConns, s.BackupDirs.AgentHostsToBackupDir, req.GetPgUpgradeVerbose(), req.GetSkipPgUpgradeChecks(), req.GetJobs(), s.Source, s.Intermediate, idl.PgOptions_check, s.Mode, pgUpgradeTimestamp)
	})
