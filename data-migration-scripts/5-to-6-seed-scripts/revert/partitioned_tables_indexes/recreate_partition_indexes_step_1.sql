...
root_partitions (relid) AS (
  ...
  FROM pg_partition
  WHERE parrelid NOT IN (SELECT oid FROM root_partitions_using_tsquery)
),
root_constraints AS (
  SELECT
    conname,
...
SET SEARCH_PATH=' || schemaname || '; ' || indexdef || ';' as definition
  FROM indexes
  WHERE (indexname, schemaname, tablename) NOT IN (SELECT conrel, relschema, rel FROM root_constraints);
