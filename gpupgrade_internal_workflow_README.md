...
1. Check for coordinator for compatibility.
...
### \*\*\*COPY coordinator to segments to bootstrap segment upgrade\*\*\*
...
1. gpugprade execute
    ...
    2. Copy the upgraded coordinator to each segment. We use the upgraded
       coordinator to bootstrap segment upgrade.
    3. Run pg_upgrade to upgrade the segments
...
1. Existing MDD files are now on segments. Issues with this as the root cause
   has manifests in several ways.
    1. xids from coordinator ends up on segments, they must be edited to to
...
1. Developers must keep in mind that pg_upgrade is run twice during execute. 
    1. This has performance impacts because master and segments cannot upgrade
       at the same time.
    1. Any pg_upgrade bottlenecks can be twice as inefficient. One such example
       of this is that checks run twice. Once on coordinator and again on
       segments. Before optimization, view checks were taking up to 2 hours for
       a single check, which meant that each check added 4 hours to run time.
1. Indexes are marked invalid on the new coordinator which are then copied to
   ...
   of relations for relation matching that happens in gen_db_file_maps.
...
scripts that fix user object to make them upgradable. Some scripts run during
...
database. We also generate the a revert script to revert objects back to their
...
#### 1a. Freezing the master after relfilenode transfer
...
the master. Freezing must happen sometime after schema is restored in
`create_new_objects`.
```
pg_upgrade on coordinator
...
...
└── freeze_master_data* <----- Freeze to make schema visibible on segments
    ...
    └── VACUUM FREEZE pg_catalog.pg_database
```
In order to make schema on the segments visible we need to freeze the master
before copying it over to segments. 

**Before:**<br>
Freezing the master was first introduced in
...
**Currrent:**<br>
The location of master freeze was later changed in
...
and minmxid. Moving master freeze to after this point is a precaution for
several reasons.
1. We ensure the gp_fastsequence values are visible by freezing the coordinator
2. One of the very last steps of coordinator pg_upgrade is freezing.

The commits that changed location of master freeze<br>
...
This doesn't seem to be used anywhere... I also fail to see how running
only checks for source cluster fits into the current gpupgrade workflow.
...
## list of issues/commits that are needed due upgrading segments using copy MDD
...
tuples exist on master but not on segments?
...
This commit introduces freezing master right before relfilenodes are
transferred. It is supposed to make the data on master visible
...
Frozenxid and the minmxid to make the tuples that came from master's
